...
ids<-read.table(paste0(path, "Account ids 1 person.txt"), stringsAsFactors = FALSE)
ids<-ids$V1
...
###################################################################################
# Ta czesc kodu ma pomagac w ustaleniu jakie obiekty sa a jakich nie ma...
...
### Nazwy wybranych kolumn
names(Merged_out)<-c("Id","Acc Parent Guardian","LiveChatTranscript","ACR person","Event","Key_User__c","Case","Lead","Opp_notWon","Opp_Won","Placement_Test_Attendance","EmailMessageRelation","Activity__c","Task")
...
names(Merged_out_loader)<-c("Id","Last_interaction_date__pc")

# write.xlsx(x = Merged_out, file = paste0(path,"dates from children",Sys.Date()," 01",".xlsx"))
write.table(Merged_out_loader, file = paste0(path,"Account loader person1_rand 19FEB.csv"), quote = TRUE, row.names = FALSE, col.names = TRUE, sep = ",", na = "")
