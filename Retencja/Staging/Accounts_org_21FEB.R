...
# Path to your folder
path<-"C:/Users/aleksander.politansk/Desktop/RFC/RFC 200/STAGING ENV WORK/ACCOUNT/"
...
ids<-read.table(paste0(path, "Account ids org.txt"), stringsAsFactors = FALSE)
ids<-ids$V1

ids_to_exclude<-read.table(paste0(path, "Ids to exclude.txt"), stringsAsFactors = FALSE)
ids<-ids[! ids %in% ids_to_exclude$V1]
...
for (i in seq(from=1,to=length(ids),by=i_rows)) {
  ...
  SELECT AccountId, LastModifiedDate
...
LiveChatTranscript_result_out <- LiveChatTranscript_result %>% 
  select(AccountId, LastModifiedDate) %>%
  group_by(AccountId) %>%
  summarise(Max_date=max(LastModifiedDate,na.rm = TRUE))  
...
for (i in seq(from=1,to=length(ids),by=i_rows)) {
  ...
  SELECT AccountId, rebuild_LastModifiedDate__c, CloseDate
  FROM Opportunity
  WHERE 
  (IsClosed = false or IsWon = false)
  AND
  AccountId in (
  "
  ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
...
j<-j+1
  ...
  Opp_notWon_work_max<-apply(Opp_notWon_result[,-1],1,max,na.rm=TRUE)
  
  
  # Prepare Opp_Part_notWon_out data frame
  Opp_notWon_out <- data.frame(Opp_notWon_result$AccountId,Opp_notWon_work_max)
  names(Opp_notWon_out) <- c("AccountId","Max_date")
  Opp_notWon_out$Max_date<-as.Date(Opp_notWon_out$Max_date)
      
  
  
Opp_notWon_result_out <- Opp_notWon_out %>% 
  select(AccountId, Max_date) %>%
  group_by(AccountId) %>%
  summarise(Max_date_distinct=max(Max_date,na.rm = TRUE))
...
for (i in seq(from=1,to=length(ids),by=i_rows)) {
  ...
  where
  Status__c != 'Withdrawn' and (Opportunity__r.IsClosed = false or Opportunity__r.IsWon = false)
  and
  Partner__c in (
  "
  ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
...
for (i in seq(from=1,to=length(ids),by=i_rows)) {
  ...
  from Opportunity_Partner__c 
  where
  Status__c != 'Withdrawn' and (Opportunity__r.IsClosed = true and Opportunity__r.IsWon = true)
  and
  Partner__c in (
  "
  ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
...
for (i in seq(from=1,to=length(ids),by=i_rows)) {
  ...
  Task_soql<-paste0("
  SELECT AccountId, rebuild_LastModifiedDate__c, ActivityDate
  from Task
  WHERE AccountId in (
  "
                           ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
...
# Take max date from extracted columns after additional work
Task_result_max<-apply(Task_result[,-1],1,max,na.rm=TRUE)


# Prepare Opp_Won_result_out data frame
Task_result_out <- data.frame(Task_result$AccountId,Task_result_max)
names(Task_result_out) <- c("AccountId","Max_date")
Task_result_out$Max_date<-as.Date(Task_result_out$Max_date)
...
Add_Organisation_Contacts_Document_Link__c_result<-data.frame()
j<-1
j_max<-ceiling(length(ids)/i_rows)

for (i in seq(from=1,to=length(ids),by=i_rows)) {
  
  Add_Organisation_Contacts_Document_Link__c_soql<-paste0("
  SELECT Organisation_Contact__c, rebuild_LastModifiedDate__c
  from Add_Organisation_Contacts_Document_Link__c
  WHERE Organisation_Contact__c in (
  "
                                      ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
                                      ")
  "
  ...
  Add_Organisation_Contacts_Document_Link__c_soql<-gsub(pattern = "\n", replacement = "", x = Add_Organisation_Contacts_Document_Link__c_soql)
  Object_Content <- sf_query(Add_Organisation_Contacts_Document_Link__c_soql)
  Add_Organisation_Contacts_Document_Link__c_result <- rbind(Add_Organisation_Contacts_Document_Link__c_result,Object_Content)
  Add_Organisation_Contacts_Document_Link__c_result<-Add_Organisation_Contacts_Document_Link__c_result %>% distinct()
  # print(paste0('Iteration number ',j," out of ",j_max," is done. Time: ",format(Sys.time(), "%X") ))
  j<-j+1
}

Add_Organisation_Contacts_Document_Link__c_result_out <- Add_Organisation_Contacts_Document_Link__c_result %>% 
  select(Organisation_Contact__c, rebuild_LastModifiedDate__c) %>%
  group_by(Organisation_Contact__c) %>%
  summarise(Max_date=max(rebuild_LastModifiedDate__c,na.rm = TRUE))  
...
Opportunity_Competitor__c2_result<-data.frame()
j<-1
j_max<-ceiling(length(ids)/i_rows)

for (i in seq(from=1,to=length(ids),by=i_rows)) {
  
  Opportunity_Competitor__c2_soql<-paste0("
  SELECT Opportunity__r.AccountId, rebuild_LastModifiedDate__c
  from Opportunity_Competitor__c
  WHERE Opportunity__r.AccountId in (
  "
                                         ,paste0("'",paste0(na.omit(ids[i:(i+(i_rows-1))]), sep = "", collapse = "','"),"'"),
                                         ")
  "
  ...
  Opportunity_Competitor__c2_soql<-gsub(pattern = "\n", replacement = "", x = Opportunity_Competitor__c2_soql)
  Object_Content <- sf_query(Opportunity_Competitor__c2_soql)
  Opportunity_Competitor__c2_result <- rbind(Opportunity_Competitor__c2_result,Object_Content)
  Opportunity_Competitor__c2_result<-Opportunity_Competitor__c2_result %>% distinct()
  # print(paste0('Iteration number ',j," out of ",j_max," is done. Time: ",format(Sys.time(), "%X") ))
  j<-j+1
}

Opportunity_Competitor__c2_result_out <- Opportunity_Competitor__c2_result %>% 
  select(Opportunity__r.AccountId, rebuild_LastModifiedDate__c) %>%
  group_by(Opportunity__r.AccountId) %>%
  summarise(Max_date=max(rebuild_LastModifiedDate__c,na.rm = TRUE))  
...
###################################################################################
#OPPORTUNITY PARTNER 2#
Opportunity_Partner__c2_result<-data.frame()
j<-1
j_max<-ceiling(length(ids)/i_rows)

for (i in seq(from=1,to=length(ids),by=i_rows)) {
  
  Opportunity_Partner__c2_soql<-paste0("
  SELECT Opportunity__r.AccountId, rebuild_LastModifiedDate__c
  from Opportunity_Partner__c
  WHERE Opportunity__r.AccountId in (
  "
  ...
  Opportunity_Partner__c2_soql<-gsub(pattern = "\n", replacement = "", x = Opportunity_Partner__c2_soql)
  Object_Content <- sf_query(Opportunity_Partner__c2_soql)
  Opportunity_Partner__c2_result <- rbind(Opportunity_Partner__c2_result,Object_Content)
  Opportunity_Partner__c2_result<-Opportunity_Partner__c2_result %>% distinct()
  # print(paste0('Iteration number ',j," out of ",j_max," is done. Time: ",format(Sys.time(), "%X") ))
  j<-j+1
}

Opportunity_Partner__c2_result_out <- Opportunity_Partner__c2_result %>% 
  select(Opportunity__r.AccountId, rebuild_LastModifiedDate__c) %>%
  group_by(Opportunity__r.AccountId) %>%
  summarise(Max_date=max(rebuild_LastModifiedDate__c,na.rm = TRUE))  
...
out_dfs<-c("Account_result_out",
"Org_Account_result_out",
"LiveChatTranscript_result_out",
"Person_Account_result_out",
"Event_result_out",
"Key_User__c_result_out",
"Case_result_out",
"Lead_result_out",
"Opp_notWon_result_out",
"Opp_Won_result_out",
"Opp_Part_notWon_out",
"Opp_Part_Won_result_out",
"Opportunity_Competitor__c_result_out",
"Opportunity_Partner__c_result_out",
"Placement_Test_Attendance__c_result_out",
"EmailMessageRelation_out",
"Activity__c_result_out",
"Task_result_out",
"Add_Organisation_Contacts_Document_Link__c_result_out",
"Opportunity_Competitor__c2_result_out",
"Opportunity_Partner__c2_result_out")

nonexisting_dfs<-out_dfs[!sapply(X = out_dfs, FUN = exists)]

print(paste0("Following _out dataframes don't exist :",nonexisting_dfs))
  
# TUTAJ STOP KATARZYNO  
###################################################################################

ob_list<-list(
# Account_result,
Org_Account_result,
# LiveChatTranscript_result,
# Person_Account_result,
Event_result,
Key_User__c_result,
Case_result,
Lead_result,
Opp_notWon_result,
Opp_Won_result,
Opp_Part_notWon_result,
Opp_Part_Won_result,
Opportunity_Competitor__c_result,
Opportunity_Partner__c_result,
# Placement_Test_Attendance__c_result,
# EmailMessageRelation_result,
Activity__c_result,
Task_result,
Add_Organisation_Contacts_Document_Link__c_result,
Opportunity_Competitor__c2_result,
Opportunity_Partner__c2_result
)
...
Merged_out<-All_ids %>% 
  ...
  left_join(Task_result_out, by = c("ids"="AccountId")) %>%
  left_join(Add_Organisation_Contacts_Document_Link__c_result_out, by=c("ids"="Organisation_Contact__c")) %>%
  left_join(Opportunity_Competitor__c2_result_out, by=c("ids"="Opportunity__r.AccountId")) %>%
  left_join(Opportunity_Partner__c2_result_out, by=c("ids"="Opportunity__r.AccountId"))

# names(Merged_out)<-c("Id","Acc Parent Guardian","ACR org","LiveChatTranscript","ACR person","Event","Key_User__c","Case","Lead","Opp_notWon","Opp_Won","Opp_Part_notWon","Opp_Part_Won","Opportunity_Competitor__c","Opportunity_Partner__c","Placement_Test_Attendance","EmailMessageRelation","Activity__c","Task","Linked Doc","Opportunity Competitor 2","Opportunity Partner 2")
names(Merged_out)<-c("Id","ACR org","Event","Key_User__c","Case","Lead","Opp_notWon","Opp_Won","Opp_Part_notWon","Opp_Part_Won","Opportunity_Competitor__c","Opportunity_Partner__c","Activity__c","Task","Linked Doc","Opportunity Competitor 2","Opportunity Partner 2")
...
write.table(Merged_out_loader, file = paste0(path,"Account loader org_rand 21FEB.csv"), quote = TRUE, row.names = FALSE, col.names = TRUE, sep = ",", na="")
